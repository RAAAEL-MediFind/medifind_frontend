...
def show_sidebar():
    with ui.left_drawer().classes(
        ...
        with ui.element('div').classes("flex flex-col flex-grow px-3 pt-6"):
            ...
            for label, route, icon_name in primary_links:
                with ui.link(route).classes("no-underline w-full"): 
                    with ui.row().classes(
                        "items-center gap-4 px-3 py-2 rounded-lg hover:bg-blue-100 transition-colors duration-200"
                    ):
            ...
            # --- Account Section ---
            ui.label("Account").classes("text-xs font-semibold text-gray-500 uppercase mt-2 mb-2 px-3")
            
            # Settings Link
            with ui.link("/settings").classes("no-underline w-full"):
                with ui.row().classes(
                    "items-center gap-4 px-3 py-2 rounded-lg hover:bg-blue-100 transition-colors duration-200"
                ):
            ...
            with ui.row().classes('w-full mt-auto mb-4 px-3'):
