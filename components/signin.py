            ...
            with ui.column().classes('w-[500px] px-8'):
                ...
                with ui.row().classes('w-full justify-start gap-4 mb-6'):
                    ...
                    with user_button:
                        ...
                        user_label = ui.label('Patient').classes('text-gray-500 font-semibold')
                ...
                with ui.column().classes("w-full relative h-[400px]"):
                    ...
                    with ui.column().classes("w-full absolute top-0 left-0 overflow-y-auto h-full pr-2") as user_form:
                        ...
                        user_password = ui.input(placeholder="Enter your password", password=True).props("outlined").classes("w-full mb-8")
                        ui.button("Sign In as Patient", on_click=lambda: handle_signin('patient')).classes("w-full bg-blue-300 text-white font-semibold py-2 rounded-full")

                    # Pharmacy Sign-in Form
                    with ui.column().classes("w-full absolute top-0 left-0 overflow-y-auto h-full pr-2") as pharmacy_form:
                        ...
                        ui.label("PASSWORD").classes("text-sm font-semibold text-gray-700")
                        pharma_password = ui.input(placeholder="Enter your password", password=True).props("outlined").classes("w-full mb-8")
                        ui.button("Sign In as Pharmacy", on_click=lambda: handle_signin('pharmacy')).classes("w-full bg-blue-300 text-white font-semibold py-2 rounded-full")
                ...
                async def handle_signin(role: str):
                    ...
                    # ##### THIS IS THE FIX: The backend expects 'email', not 'username' #####
                    payload_data = {'email': email, 'password': password}
                    
                    ui.notify('Signing in...', spinner=True)
                    response = await asyncio.to_thread(send_request_sync, endpoint_url, payload_data)
                    ...
                    if isinstance(response, requests.Response):
                        if response.status_code == 200:
                            ...
                            ui.notify(f"Login failed: {error_detail}", color='negative', multi_line=True)
                    else:
                        ui.notify(f"Could not connect to the server: {response}", color='negative')

                # Toggle Logic for showing the correct form
                ...
                def select_role(role):
                    ...
                    # Correctly use 'user' for the button logic, but send 'patient' to backend
                    if role == 'user':
                ...
                # The role for the UI is 'user', but we will send 'patient' to the backend
